...
@Component({
  selector: 'app-admin-sales-recruiters',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatTableModule,
  ...
  templateUrl: './admin-sales-recruiters.component.html',
  styleUrl: './admin-panel.scss'
})
export class AdminSalesRecruitersComponent implements OnInit {
  private readonly admin = inject(AdminService);
  private readonly toast = inject(ToastrService);
  private readonly fb = inject(FormBuilder);
  cols = ['userEmployeeId', 'name', 'email', 'phone', 'assigned', 'status', 'actions'];
  ...
  private refreshUserOptions(): void {
    ...
    this.userOptions = this.allUsers.filter(
      (u) => u.roles.some((r) => r === 'SalesRecruiter') && !taken.has(u.id)
    );
  }
    ...
    this.admin
      ...
      .subscribe({
        next: () => {
          ...
          this.form.reset({ status: 'Active', userId: '', firstName: '', lastName: '', email: '', phoneNumber: '' });
          this.reload();
        },
        error: (e: { error?: { message?: string } }) => this.toast.error(e?.error?.message ?? 'Error')
      });
